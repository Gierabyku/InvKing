...
const AiModal: React.FC<AiModalProps> = ({ isOpen, onClose, tips, isLoading }) => {
    ...
    const formattedTips = tips.split('\n').map((line, index) => {
        if (line.startsWith('**')) {
            return <h4 key={index} className="font-bold text-indigo-400 mt-4 mb-2">{line.replace(/\*\*/g, '')}</h4>;
        }
        line = line.replace(/^\s*-\s*/, ''); // Remove leading dash
        if (line.trim() === '') return null;
        return (
            <li key={index} className="flex items-start mb-2">
                <span className="text-gray-500 mr-3 mt-1 text-xs"><i className="fas fa-circle"></i></span>
                <span>{line}</span>
            </li>
        );
    });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md max-h-[80vh] flex flex-col transform transition-all animate-fade-in-up">
                <div className="flex justify-between items-center mb-4">
                     <h2 className="text-xl font-bold text-white flex items-center"><i className="fas fa-lightbulb text-yellow-400 mr-3"></i>Sugestie Diagnostyczne AI</h2>
                ...
                <div className="overflow-y-auto pr-2 text-gray-300">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center h-48">
                            ...
                            <p className="mt-4 text-gray-400">Analizuję problem...</p>
                        </div>
                    ) : (
